...
*/
if ( ! empty($hu_settings['maintenance_mode']) ){

  include_once HU_OPTIONS_PLUGIN_PATH.'helper_functions.php';

  function maintenance_mode_lockout_notice() {
    if( ! is_hosting_utilities_user() && !is_admin()) {
  ...
  function hu_migration_mode_warning() {
  ...
  add_action( 'admin_notices', 'hu_migration_mode_warning' );
}

/* Once the HSTS header has been sent, the browser will remember it for 1 year, and there is no way of undoing this */
if ( ! empty($hu_settings['hsts']) && $hu_settings['hsts'] === true ){
	function hu_add_hsts_header() {
	...
	add_action( 'send_headers', 'hu_add_hsts_header' );
}
