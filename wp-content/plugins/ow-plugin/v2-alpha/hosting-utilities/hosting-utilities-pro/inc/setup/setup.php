...
*/
function hu_mark_setup_stage_as_successful($stage=null){

	static $stages = array(
		'new_stages' => HU_SETUP_VERSIONING['NEW_SETUP_STAGES'],
		'htaccess' => false,
	...
	static $versions = array(
		'new_stages' => HU_SETUP_VERSIONING['NEW_SETUP_STAGES'],
		'htaccess' => HU_SETUP_VERSIONING['HTACCESS'],
		'maintenance_dropin' => HU_SETUP_VERSIONING['MAINTENANCE_DROPIN'],
		'branding_info' => HU_SETUP_VERSIONING['BRANDING_INFO'],
		'logo' => 1,
		'add_caps' => HU_SETUP_VERSIONING['ADD_CAPABILITIES'],
		'mu_plugin' => HU_SETUP_VERSIONING['MU_PLUGIN']
	);
...
function insert_with_markers_top_of_file( $filename, $marker, $insertion ) {
    ...
    if ( $existing_lines === $insertion ) {
        ...
        fclose( $fp );
				...
				hu_mark_setup_stage_as_successful('htaccess');
        return true;
    }
...
*/
function hu_store_branding_info(){
	global $hu_branding_defaults;
	...
	$wl_info = $data[0]['acf']['whitelabel'] ?: $hu_branding_defaults;

	update_option('hu_whitelabel_info', $wl_info);
	hu_mark_setup_stage_as_successful('branding_info');
}

/*
	Store's the logo in the `hu_site_logo` variable
*/
function hu_store_logo(){
	require_once HU_PRO_PLUGIN_PATH.'wp_authenticator/auth.php';
  ...
  $data = json_decode($json, true);
	...
	update_option( 'hu_site_logo', $logo, false );
	hu_mark_setup_stage_as_successful( 'logo' );
}
...
*/
function hu_maintenance_drop_in(){
	$content = '<?php $maintenance_page = "' . HU_OPTIONS_PLUGIN_PATH . 'inc/lockout_notices/updating_page.php";';
	$content .= <<<'CODE'
...
CODE;
	...
	fwrite($fp, $content);
	fclose($fp);
	hu_mark_setup_stage_as_successful('maintenance_dropin');
}

/*
 * Add our rewrite rules to the htaccess file
*/
function hu_htaccess_rewrites(){
...
function hu_mu_plugin(){

	$content = '<?php $mu_page = "' . addslashes(HU_PRO_PLUGIN_PATH . 'inc/mu_plugin/mu_plugin.php') . '";';
	$content .= <<<'CODE'
/**
...
CODE;
	...
	hu_mark_setup_stage_as_successful('mu_plugin');
}
...
*/
function hu_add_capabilities(){
	$editor_role = get_role( 'editor' );
	if ($editor_role) /* $editor_role was null on curecreteconnect.com, I guess it's possible to remove the editor role from your WordPress installation*/
		$editor_role->add_cap( 'hu_submit_tickets' );
	$admin_role = get_role( 'administrator' );
	$admin_role->add_cap( 'hu_submit_tickets' );
	hu_mark_setup_stage_as_successful( 'add_caps' );
}
