...
// TODO do this sooner than DOM ready, but after we are sure that the menu is completely created
function hu_modify_admin_menu() {
  ?>
  	<style>
		#adminmenu{
			visibility: hidden;
		...
		#adminmenu-search{
			...
			background: #373f4669;
			color: #ddd;
			border-top: 1px solid #5556;
		}
		#adminmenu-search:focus {
		    box-shadow: none;
		}
		...
		#adminmenu.searching > .wp-menu-separator{
			display: none;
		}
	</style>
	...
	document.addEventListener("DOMContentLoaded", function(e) {
	  ...
	  // Hide menus that are being displayed in the Hosting Utilities menu
	  let hide_these_menus = [
		  // Ewww
		  '#adminmenu li:not(.toplevel_page_wpoverwatch) a[href="options-general.php?page=ewww-image-optimizer/ewww-image-optimizer.php"]',
		  // Breeze
		  '#adminmenu li:not(.toplevel_page_wpoverwatch) a[href="options-general.php?page=breeze"]',
		  // Wordfence
		  '#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="admin.php?page=Wordfence"]',
		  // WP Mail SMTP
		  '#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="options-general.php?page=wp-mail-smtp"]',
		  // WP Time Capsule
		  '#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="admin.php?page=wp-time-capsule-monitor"]',
		  // TODO test that these are in hosting the utilities menu before hiding
		  // Swift Performance
		  //'#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="tools.php?page=swift-performance"]',
		  // WebP Express
		  //'#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="options-general.php?page=webp_express_settings_page"]',
		  // Redis
		  //'#adminmenu > li:not(.toplevel_page_wpoverwatch) a[href="options-general.php?page=redis-cache"]',
	  ]

	  for (css_selector of hide_these_menus){
		  let el = document.querySelector(css_selector)
		  if (el && ! el.classList.contains('wp-menu-open') && ! el.classList.contains('current'))
	  ...
	  searchEl.type = "text";
	  searchEl.id = 'adminmenu-search';
	  searchEl.placeholder = 'Search';
	  searchEl.tabIndex = 1;
	  adminmenu.insertAdjacentElement('afterbegin', searchEl);

	  /* Make searchbar work */
	  searchEl.addEventListener('input', function(ev){
		  if (ev.target.value){
			  adminmenu.classList.add('searching')
		  }
		  menus = document.getElementsByClassName('menu-top')
		  for (menu of menus){
			  menu.classList.add('hide');

			  if ( menu.innerText.toLowerCase().includes(ev.target.value.toLowerCase()) ){
...
add_action('admin_head', 'hu_modify_admin_menu');
