...
class CD_APD_Core
{
	...
	/* WP Overwatch hacking: adding a function to help us look inside trunk directories */
	private function get_plugins_from_trunk($plugin_root){
				...
				if ( is_dir( "{$plugin_root}/{$file}" ) )
				{
					...
					if ( $plugins_subdir )
					{
						while ( ( $subfile = readdir( $plugins_subdir ) ) !== false )
						{
							...
							/* WP Overwatch hacking: Go one directory deep on trunk folders */
							if ($subfile === 'trunk'){
								$trunk_plugins = $this->get_plugins_from_trunk("{$plugin_root}/{$file}/{$subfile}");
								foreach($trunk_plugins as $plugin){
									$plugin_files[] = "$file/trunk/{$plugin}";
								}
							}
						}
