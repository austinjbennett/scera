...
if (is_ow_page()){
	function ow_add_body_classes($classes){
		$ow_page_prefix = strtolower( ow_branding('url friendly') );
		$add_class = str_replace($ow_page_prefix, 'wpoverwatch', strtolower($_REQUEST['page']) );
		return $classes . ' ' . $add_class . ' ';
	}
	add_filter( 'admin_body_class', 'ow_add_body_classes', 999 );
}
...
if ( ! empty($wpoverwatch_settings['migration_mode']) ){
  ...
  function migration_mode_lockout_notice() {
    if( ! is_wpoverwatch_user()) {
      require OW_PLUGIN_PATH.'/inc/lockout_notices/migration_page.php'; /* Not sure if its /inc/ or inc/ */
      exit();
    }
  }
  add_action('init', 'migration_mode_lockout_notice'); /* init is the earliest we can check if we are the wpoverwatch user */

  function ow_maintenance_mode_warning() {
    if( is_wpoverwatch_user()) {
      ?>
  ...
  add_action( 'admin_notices', 'ow_maintenance_mode_warning' );
}
...
if ( ! empty($wpoverwatch_settings['hide_update_notices']) ){
	...
	function ow_hide_update_notices() {
    if( is_wpoverwatch_user()) {
      ?>
      <script>
  ...
  add_action( 'admin_notices', 'ow_hide_update_notices', 99999 );
}
...
if(preg_match('~(.wp-overwatch.com)~', home_url())) {
  ...
  add_action( 'admin_notices', 'ow_staging_site_warning' );
}
*/
/* dev sites */
/*
if(preg_match('~(.dev|.local|.ow)~', home_url())) {
  ...
  add_action( 'admin_notices', 'ow_dev_site_warning' );
}
*/
