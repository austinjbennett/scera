...
if( is_admin() ){
    ...
    function ow_hidden_plugins_notice() {
        global $pagenow;
        ...
        $hosting_company = get_option('ow_whitelabel_info', false)['default'] ?? 'WP Overwatch';
    ...
    add_action( 'admin_notices', 'ow_hidden_plugins_notice' );

  function ow_unload_plugins( $plugins ){
    global $ow_removed_plugins, $pagenow;
    ...
    foreach ( $unnecessary_plugins as $plugin_name => $plugin_path ) {
      ...
      if( $k !== false ){
        $ow_removed_plugins[$plugin_name] = $plugin_path;
  ...
  add_filter( 'option_active_plugins', 'ow_unload_plugins' );
