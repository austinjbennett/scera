...
const VALID_OW_DASHBOARD_CAPABILITIES = array('edit_dashboard', 'overwatch_dashboard');
...
*/
function get_ow_api_key($website=null, $wpDir="../wp-load.php"){
  $user = null;
  ...
  if (! (ow_check_capabilities($user, VALID_OW_DASHBOARD_CAPABILITIES)) ){
    exit("Insufficent privelages");
  }

  return get_option('ow_admin_settings', array())['api_key'];
...
/* deprecated. Alias of get_ow_api_key */
function get_token() {
  return call_user_func_array("get_ow_api_key", func_get_args());
}

/* Returns true if the user has a capability in $caps, otherwise, returns false */
function ow_check_capabilities($user, $caps){
  foreach ($caps as $cap){
