...
function rpgc_update_card( $order_id ) {
	...
	$giftcard = new KODIAK_Giftcard();
	$orderPayment = $giftcard->wpr_get_payment_amount();
	$giftcardsUsed = array();

	if ( isset( $giftCards ) ) {
		foreach ($giftCards as $key => $giftCard_id ) {
			if ( $giftCard_id != '' ) {
				$giftcardBalance = wpr_get_giftcard_balance( $giftCard_id );

				//Decrease Balance of card
				if ( $orderPayment > 0 ) {
					$newGiftcardBalance = $giftcard->wpr_decrease_balance( $giftCard_id, $orderPayment );
					$amountApplied = $giftcardBalance - $newGiftcardBalance;
					$orderPayment -= $amountApplied;
					$giftcardsUsed[ $giftCard_id ] = array(  "balance" => $newGiftcardBalance,
                                       															"used" => $amountApplied );
				}
	...
	// Update Gift Card Balances on Orders
	$giftCardBalances = get_post_meta( $order_id, 'rpgc_balance', true );
	$giftCardPayments = get_post_meta( $order_id, 'rpgc_payment', true );
	$giftCardIDsUsed = get_post_meta( $order_id, 'rpgc_id', true );

	foreach( $giftcardsUsed as $key => $giftcard ) {
		if ( is_array ( $giftCardBalances ) ) {
    	$giftCardBalances[] = $giftcard["balance"];
	  } else {
	    $giftCardBalances = array( $giftcard["balance"] ) ;
	  }

		if ( is_array ( $giftCardPayments ) ) {
		  $giftCardPayments[] = $giftcard["used"];
		} else {
		  $giftCardPayments = array( $giftcard["used"] );
		}

		$giftCard_IDs = get_post_meta ( $key, 'wpr_existingOrders_id', true );
    if ( is_array ( $giftCard_IDs ) ) {
      ...
      $giftCard_IDs = array( $order_id );
		...
		if ( is_array ( $giftCardIDsUsed ) && ( $giftcard["used"] > 0 ) ) {
		  $giftCardIDsUsed[] = $key;
		} else {

		  $giftCardIDsUsed = array( $key );
		}
		update_post_meta( $key, 'wpr_existingOrders_id', $giftCard_IDs ); // Saves order id to gifctard post
		update_post_meta( $key, 'rpgc_balance', $giftCardBalances );
	}
	update_post_meta( $order_id, 'rpgc_balance', $giftCardBalances );
	update_post_meta( $order_id, 'rpgc_id', $giftCardIDsUsed );
...
function wpr_notate_gift_card( ) {

}

function wpr_notate_order( ) {

}


			// Note Card with order information
			// Note Order with Card information
